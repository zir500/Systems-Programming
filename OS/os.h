...
typedef struct {
	...
	void 							(* wait_callback)			(OS_TCB_t * const task, uint32_t const checkCode);
	void 							(* notify_callback)		(OS_TCB_t * const task);
	void 							(* sleep_callback)		(OS_TCB_t * const task, uint32_t const wakeupTime);
} OS_Scheduler_t;
...
/* Initialises a task control block (TCB) and its associated stack.  The stack is prepared
   ...
   The third argument is a pointer to the function that the task should execute.
	 The Fourth argument is the priority of the task (In whatever context
   The Fifth argument is a void pointer to data that the task should receive. */
void OS_initialiseTCB(OS_TCB_t * TCB, uint32_t * const stack, void (* const func)(void const * const),	uint32_t const priority, 
...
void __svc(OS_SVC_WAIT) OS_wait(uint32_t const checkCode);
